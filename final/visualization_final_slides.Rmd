---
title: "Competition in the NFL: </br> The Patriots are actually very good at football"
...
top_teams_by_year <- nfl %>%
    group_by(season) %>%
    select(season, team, div_general,WLRatio) %>% 
    top_n(n=4) 
)
top_teams_by_year_summary <- top_teams_by_year %>%
    group_by(div_general) %>%
    summarise(n = n())
names(top_teams_by_year_summary) <- c("Division", "Number of Teams")
...
- 26 variables and 320 observations (10 years of data on 32 teams)
...
- Historical consistency of teams -- how likely are rank swings?
...
- total offensive and defensive yards
...
- Is either division dominant at the top levels?
...
nfl %>%
    ggplot(aes(x=WLRatio, y=as.factor(season), fill = div_general)) +
        ...
        labs(x = "Win/Loss Ratio", y = NULL, title = "Team Records Over Time", 
             fill = "Division") +
        theme_gray()
```
...
- Let's look more carefully at what's going on at the "top" of that last graph.
- Take the top 4 each year (by W/L ratio), is there a division that wins out?

<p class="incremental"> 
```{r, echo = FALSE, message = FALSE}
top_teams_by_year_summary %>% 
  ggplot(aes(x="", y=`Number of Teams`, fill=Division)) +
    geom_bar(width = 1, stat = "identity") +
    coord_polar("y", start=0) +
    theme_void() +
    labs(title = "Who's winning? Percentage of teams in the top four")
```
</p>
...
- Is the NFL competitive in terms of rankings movement across seasons?
...
- Outliers + variation = chance for swings between seasons
...
- Are play styles similar across all teams?
...
- The top teams pass slightly more than the league average
    - Probably overall good for the "health" of the game
    - Passing plays are more exciting and allow for bigger swings.
    - Carolina's weird.
- The deviations aren't large. 
...
```{r, echo = FALSE}
knitr::include_graphics("https://www.telegraph.co.uk/content/dam/american-football/2018/10/05/TELEMMGLPICT000171842098_trans_NvBQzQNjv4BqI3FCE1f7boN3_RkHZJyAaYRkJANvUDXYXD1hoFS_yMY.jpeg?imwidth=480")
```
...
-  The gradient is as we expect
    - Being worse at either typically hurts records
    
    
```{r, echo = FALSE, message = FALSE, out.height = '70%'}
...
ggplot(nfl, aes(x=total_defensive_allowed, y=total_offensive_gained, col = record, label = team)) + 
  geom_point() + 
  scale_color_manual(breaks = c("Double Positive", "Positive", "Negative", "Double Negative"), values = c("green", "gold", "darkorange1", "red")) + 
  labs(x = "Total Defensive Yards Allowed in Season", 
       y = "Total Offensive Yards Gained in Season", 
       col = "Win Record",
       title = "Offense vs. Defense for Team Strength") + 
...
- What separates a good team from a great team?
    - Sometimes defensive improvement, typically offensive
...
- Passing vs Running and Offense vs Defense stats would be more informative if per-game
    - More information on team-level match-ups

- Only have data since 2009. Older data might look different.

- In addition to win/loss, would have liked overall rank in season
    - More direct way of comparing teams
...
- What is the relationship between offense/defense and win probabilities?
